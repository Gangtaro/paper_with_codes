...
from models import SASRecExplicit
from trainers import ExplicitTrainer
from utils import (
    EarlyStopping,
    check_path,
...
def main():
    ...
    user_seq, rating_seq, max_item, valid_rating_matrix, test_rating_matrix, _ = get_user_seqs(
        args, 
        is_train = True # (True: LableEncoder가 실행된다. / False: 저장된 LableEncoder를 불러온다.)
    )
    ...
    model = SASRecExplicit(args=args)

    trainer = ExplicitTrainer(
        model, train_dataloader, eval_dataloader, test_dataloader, None, args
    )

    # -- load pretrain if exist
